defmodule Day6 do
  ...
  def cycle?(fast, slow, itr, grid, fast_dir \\ :up, slow_dir \\ :up) do
    IO.inspect({fast, "FAST"}, charlists: :as_numbers)
  ...
  # TODO: I NEED TO ADD DIRECTIONALITY TO THIS FUNCTION
  defp oob?([x, y], grid) do
    IO.puts("OOB")
    IO.inspect([x <= -1, y <= -1, oob_wide?([x, y], grid)])
    IO.inspect([x, y], charlists: :as_numbers)
    x <= -1 or y <= -1 or oob_wide?([x, y], grid)
  end
  ...
  defp oob_wide?([x, y], grid) do
    ...
    # [h | _rest] = grid
    IO.inspect({h, "head"})
    IO.inspect({x, y, "OOB_WIDE x, y"})
    len_x = length(h)
    len_y = length(grid)
    IO.inspect({"len_x", len_x, "len_y", len_y})
...
defmodule Coordinates do
  ...
  # TODO: THIS MIGHT BE GOING OVERBOARD BECAUSE I"M ALREADY PASSING IN THE NEXT VALUES
  # I NEED TO DECOUPLE THE NAVIGATION FROM THE CHARACTER CALCULATOR
  def next([x, y], grid, :fast, :down) do
    IO.puts("MIGHT BE BREAKING2")
